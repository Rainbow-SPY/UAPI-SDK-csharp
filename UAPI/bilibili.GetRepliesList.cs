    ...
    public partial class bilibili
    {
        ...
        public static async Task<RepliesListType> GetRepliesList(string oid, string sort = "0", int ps = 20, int pn = 1)
        {
            var (result, statusCode) = await Interface.GetResult<RepliesListType>(
                $"{requestUrl_Main}replies?oid={oid}&sort={sort}&ps={ps}&pn={pn}");
            if (!Interface.IsGetSuccessful(result,"oid",statusCode,new IException.bilibili.BilibiliServiceError(),"bilibili replies",IException.bilibili._Bilibili_Service_Error))
