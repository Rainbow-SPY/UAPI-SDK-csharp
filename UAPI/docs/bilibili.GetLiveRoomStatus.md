...
| `long` **LiveroomID** | 直播间的真实房间号（长号） | |
| `long` **short_id** | 直播间的短号（靓号）如果没有设置，则为0 | |
| `long` **Fans** | 主播的粉丝数（关注数量） | |
| `long` **PopularValue** | 直播间当前的人气值, 注意! 这不是真实在线人数 | |
| `bool` **is_portrait** | ??? | ??? |
...
| `int` **area_name** | 子分区名称 | |
| `int` **area_id** | 子分区ID | |
| `string` **BackgroundImageUrl** | 直播间背景图的URL | |
| `string` **title** | 当前直播间的标题 | |
| `string` **CoverImageUrl** | 用户设置的直播间封面URL | |
