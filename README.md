...
- <img src="UAPI/docs/rainbow-spy.jpg" alt="" width="55" height="51.2"/>&nbsp;[Rainbow-SPY](https://github.com/Rainbow-SPY)
- <img src="UAPI/docs/shuakami.jpg" alt="" width="55" height="51.2"/>&nbsp;[Shuakami](https://github.com/shuakami/)
...
4. [杂项](#杂项)
    ...
    2. [天气请求](#天气请求)
    3. [获取全球时区时间](#获取全球时区时间)
    4. [获取手机号码的归属地信息](#获取手机号码的归属地信息)
    5. [获取一组随机数字](#获取一组随机数字)
    6. [获取程序员历史上的今天的事件](#获取程序员历史上的今天的事件)
    7. [转换 Unix 时间戳](#转换-unix-时间戳)
    8. [通过快递单号识别快递公司](#通过快递单号识别快递公司)
    9. [查询农历时间](#查询农历时间)
    10. [获取支持的快递公司列表](#获取支持的快递公司列表)
    11. [查询快递物流信息](#查询快递物流信息)
    12. [计算两个日期之间的差值](#计算两个日期之间的差值)
...
* 参数选项:
    ...
    * **sort:** 指定查询视频的排序方式。支持 `0/time（按时间）`、`1/like（按点赞）`、`2/reply（按回复数）`、
      `3/hot/hottest/最热（按最热）`。默认为`0/time`。
    * **ps:** 每页获取的评论数量，范围是`1`到`20`。默认为 `20`。
    * **pn:** 要获取的页码，从`1`开始。默认为 `1`。
